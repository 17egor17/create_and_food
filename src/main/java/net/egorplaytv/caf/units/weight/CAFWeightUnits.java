package net.egorplaytv.caf.units.weight;
...
public class CAFWeightUnits {
    private float weight;

    public static CAFWeightUnits EMPTY = new CAFWeightUnits(0F);

    public CAFWeightUnits(float weight) {
        this.weight = weight;
    }

    public CAFWeightUnits(CAFWeightUnitsInteger weight) {
    ...
    public static CAFWeightUnits fromJson(JsonObject json) {
        return new CAFWeightUnits(GsonHelper.getAsFloat(json, "weight", 100));
    }

    public static CAFWeightUnits fromNetwork(FriendlyByteBuf buf) {
        return new CAFWeightUnits(buf.readFloat());
    }

    public static void toNetwork(FriendlyByteBuf buf, CAFWeightUnits value) {
        buf.writeFloat(value.getWeight());
    }

    public CompoundTag writeToNBT(CompoundTag nbt) {
        nbt.putFloat("CAFWeightUnits", getWeight());
        return nbt;
    }

    public static CAFWeightUnits loadFromNBT(CompoundTag nbt) {
        if (nbt == null)
            return CAFWeightUnits.EMPTY;
        CAFWeightUnits energyUnits = new CAFWeightUnits(nbt.getFloat("CAFWeightUnits"));
    ...
    public CAFWeightUnits copy() {
        return new CAFWeightUnits(weight);
    }


    public static final Map<Item, CAFWeightUnits> WEIGHT_ITEMS = Maps.newLinkedHashMap();

    public static Map<Item, CAFWeightUnits> getWeightItems(){
        copy(CAFWeightUnitsInteger.getWeightItems());
        return WEIGHT_ITEMS;
    }

    private static void addI(TagKey<Item> pItemTag, float weight) {
        for(Holder<Item> holder : Registry.ITEM.getTagOrEmpty(pItemTag)) {
            if (weight > 0) {
                WEIGHT_ITEMS.put(holder.value(), new CAFWeightUnits(weight));
            }
        }

    }

    private static void addI(ItemLike pItem, float weight) {
    ...
    private static void addB(TagKey<Block> pBlockTag, float weight) {
        for(Holder<Block> holder : Registry.BLOCK.getTagOrEmpty(pBlockTag)) {
            if (weight > 0) {
                WEIGHT_ITEMS.put(holder.value().asItem(), new CAFWeightUnits(weight));
            }
        }
    }

    private static void copy(Map<Item, CAFWeightUnitsInteger> weight) {
        for (Item item : weight.keySet()) {
            WEIGHT_ITEMS.put(item, new CAFWeightUnits(weight.get(item)));
        }
    }

    public static class CAFWeightUnitsInteger {
        private int weight;

        public static CAFWeightUnitsInteger EMPTY = new CAFWeightUnitsInteger(0);

        public CAFWeightUnitsInteger(int weight) {
            this.weight = weight;
        }
        ...
        public static CAFWeightUnitsInteger fromJson(JsonObject json) {
            return new CAFWeightUnitsInteger(GsonHelper.getAsInt(json, "weight", 100));
        }

        public static CAFWeightUnitsInteger fromNetwork(FriendlyByteBuf buf) {
            return new CAFWeightUnitsInteger(buf.readInt());
        }

        public static void toNetwork(FriendlyByteBuf buf, CAFWeightUnitsInteger value) {
            buf.writeInt(value.getWeight());
        }

        public CompoundTag writeToNBT(CompoundTag nbt) {
            nbt.putInt("CAFWeightUnits", getWeight());
            return nbt;
        }

        public static CAFWeightUnitsInteger loadFromNBT(CompoundTag nbt) {
            if (nbt == null)
                return CAFWeightUnitsInteger.EMPTY;
            CAFWeightUnitsInteger energyUnits = new CAFWeightUnitsInteger(nbt.getInt("CAFWeightUnits"));
        ...
        public CAFWeightUnitsInteger copy() {
            return new CAFWeightUnitsInteger(weight);
        }


        public static final Map<Item, CAFWeightUnitsInteger> WEIGHT_ITEMS = Maps.newLinkedHashMap();

        public static Map<Item, CAFWeightUnitsInteger> getWeightItems(){
        ...
        private static void addI(TagKey<Item> pItemTag, int weight) {
            for(Holder<Item> holder : Registry.ITEM.getTagOrEmpty(pItemTag)) {
                if (weight > 0) {
                    WEIGHT_ITEMS.put(holder.value(), new CAFWeightUnitsInteger(weight));
                }
            }
        ...
        private static void addI(ItemLike pItem, int weight) {
            Item item = pItem.asItem();
            if (weight > 0) {
                WEIGHT_ITEMS.put(item, new CAFWeightUnitsInteger(weight));
            }
        }
        ...
        private static void addB(TagKey<Block> pBlockTag, int weight) {
            for(Holder<Block> holder : Registry.BLOCK.getTagOrEmpty(pBlockTag)) {
                if (weight > 0) {
                    WEIGHT_ITEMS.put(holder.value().asItem(), new CAFWeightUnitsInteger(weight));
                }
            }
        }
    }
