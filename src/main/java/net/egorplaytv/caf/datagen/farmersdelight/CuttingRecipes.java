...
public class CuttingRecipes {
    private static void cutting(Consumer<FinishedRecipe> pConsumer) {
        CuttingBoardRecipeBuilder.cuttingRecipe(Items.EGG, CAFTags.Items.TOOLS_KNIVES, CAFItems.RAW_EGG.get())
                .addResult(CAFItems.EGG_SHELL.get())
                .save(pConsumer, getCutting(getRecipeId(Items.EGG)));

        CuttingBoardRecipeBuilder.cuttingRecipe(CAFItems.PIZZA.get(), CAFTags.Items.TOOLS_KNIVES, CAFItems.PIZZA_SLICE.get(), 8)
                .save(pConsumer, getCutting(getRecipeId(CAFItems.PIZZA_SLICE.get())));

        CuttingBoardRecipeBuilder.cuttingRecipe(AllItems.DOUGH.get(), CAFTags.Items.TOOLS_KNIVES, CAFItems.SMALL_DOUGH.get(), 4)
